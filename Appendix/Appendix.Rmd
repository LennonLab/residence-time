---
title: "A residence time theory for biodiversity: Appendices and supplemental results"
...
# Appendix B: ODD protocol for describing individual-based models (IBM)
...
## B4. System level state variables
...
Resource particle size & Range of sizes for inflowing resource particles & 1 - 100 \\[3pt]
...
## B5. Process overview and scheduling
...
## B7. Other design concepts of the ODD protocol
...
### RAD-Data.csv
A file holding species abundance data for each species in each IBM, at each of 100 recorded time points. This file is useful for analyzing patterns of community structure such as rank-abundance distributions and for analyzing compositional changes across time.

### active.RAD-Data.csv
Similar to the RAD-Data.csv file above, but constrained to only the active abundances of species.

### dormant.RAD-Data.csv
Similar to the RAD-Data.csv file above, but constrained to only the dormant abundances of species.


## B9. Metrics quantified by the IBM platform
...
## B10. General ecological patterns generated by the IBM platform

The IBM source code includes python scripts to analyze the following ecological patterns.
...
*Diversity-abundance scaling relationships* -- Aspects of species-level diversity have been shown to scale with the total abundance  ($N$) of individuals in a sample at rates that are often similar for microorganisms and macroscopic plants and animals (Locey and Lennon 2016). These aspects include the following: Richness ($S$), or the number of species in a community. Rarity, or the concentration of species at low abundances. Absolute dominance ($N_{max}$), or the greatest species abundance. Evenness ($E$), or the similarity or equitability of abundance among species.
...
## Figure S1: Species abundance distribution (SAD)
Kernel density curves show the distribution of r-square values for fits of maximum likelihood forms of two SAD models: the Poisson lognormal distribution (red) and the log-series distribution (blue). The r-square value reveals the percent of variation in the SADs of IBMs that is explained by the SAD models. As commonly observed in ecological communities, both models often explain >75% variation in abundance among species.


```{r fig.width=8,echo=FALSE,fig.align="center"}
library(png)
library(grid)
img <- readPNG("Supplement/SupFig1.png")
grid.raster(img)
```


## Figure S2: Taylor's Law
Our IBM platform reproduced the scaling of variance in population size with average population size, i.e., Taylor's Law. The scaling exponents for IBMs were well within the observed range of values for Taylor's Law (between 1 and 2). Each data point represents a single species. These results were consistent regardless of whether we focused on species' dormant abundances, active abundances, or total abundances. Rainbow spectrum colors represent species belonging to systems of different $\tau$, with red being shortest and violet being longest.

```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig2.png")
grid.raster(img)
```
\newpage
 
 
## Figure S3: Diversity-abundance relationships  
Our IBM platform reproduced the diversity-abundance scaling relationships of Locey and Lennon (2016). Each data point represent the mean for a single model. Rainbow spectrum data points represent systems of different $\tau$, with red being shortest and violet being longest.


```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig3.png")
grid.raster(img)
```
\newpage


## Figure S4: Metabolic scaling
Our IBM platform closely reproduced the 3/4 power scaling of basal metabolic rate to body size for metabolically active individuals. This same scaling does not hold for metabolically dormant individuals, as might be expected. Each data point represent the mean for a single model. Rainbow spectrum data points represent systems of different $\tau$, with red being shortest and violet being longest.

```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig4.png")
grid.raster(img)
```
\newpage


## Figure S5: Residence time does not approximate individual age. 
In idealized systems such as chemostats, residence time ($\tau$) approximates the average amount of time that passively dispersing individuals remain in a system. This prediction failed in our IBMs, which simulated systems that lack stable-state dynamics and that were characterized by immigration and stochastic dynamics. Each data point represent the mean for a single model.

```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig8.png")
grid.raster(img)
```
\newpage

## Figure S6: Stochastic population dynamics
Time series of populations from different IBMs. These populations maintained non-zero abundances for the duration of their IBM and are clearly characterized by non-stable state dynamics, stochastic changes, and highly fluctuating population sizes. 

```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig7.png")
grid.raster(img)
```


## Figure S7: Dilution rate does not approximate growth rate.
In idealized systems such as experimental chemostats, diluation rate ($1/\tau$) approximates specific growth rate ($\mu$), that is, the proportional rate of population growth. However, this expected relationship could be expected to fail in systems that lack stable-state dynamics and that are characterized by stochastic population and community dynamics, as in our IBMs. Each data point represent the mean for a single model.

```{r fig.width=8,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig6.png")
grid.raster(img)
```
...
## Figure S8: Opposing influence of flow rate and system size
According the definition of residence time as the ratio of a system's size $V$ to its rate of flow $Q$, $V$ and $Q$ have opposing influences on the value of residence time. Hence, $V$ and $Q$ should have opposing influences on variables that respond to residence time. Each data point represent the mean for a single model. Rainbow spectrum data points represent systems of different $V$ (left column) or different $Q$ (right column), with red representing the smallest and violet representing the largest.

```{r fig.height=7,echo=FALSE,fig.align="center"}
img <- readPNG("Supplement/SupFig5.png")
grid.raster(img)
```
\newpage
...
1. Brown, J. H., Gillooly, J. F., Allen, A. P., Savage, V. M., and G. B. West. 2004. Toward a 	metabolic theory of ecology. Ecology, 85:1771-1789.

2. Crow, E. L., & K. Shimizu. 1988. Lognormal Distributions: Theory and Applications. New York: Markel Dekker.

3. Glazier, D. S. 2006. The 3/4-power law is not universal: evolution of isometric, ontogenetic 	metabolic scaling in pelagic animals. AIBS Bulletin, 56:325-332.

4. Grimm, V., Revilla, E., Berger, U., Jeltsch, F., Mooij, W. M., Railsback, S. F., Thulke, H., 	Weiner, J., Wiegand, T., and D. L., DeAngelis. 2005. Pattern-oriented modeling of 	agent-based complex systems: lessons from ecology. Science, 310:987-991.

5. Harte, J. 2011. Maximum entropy and ecology: a theory of abundance, distribution, and 	energetics. Oxford University Press.

6. Hubbell, S. P. 2001. The unified neutral theory of biodiversity and biogeography. Princeton	University Press.

7. Locey, K. J., and J. T., Lennon. 2016. Scaling laws predict global microbial diversity. Proceedings of the National Academy of Sciences of the United States of America, 113:5970-5975.

8. Locey K.J., and J. T. Lennon. 2017. A modeling platform for the simultaneous emergence of ecological patterns. PeerJ Preprints 5:e1469v3

9. Locey, K. J., Fisk, M. C., and J. T. Lennon. 2017. Microscale Insight into Microbial Seed Banks. Frontiers in Microbiology, 7:2040.

10. McGill, B. J., Etienne, R. S., Gray, J. S., Alonso, D., Anderson, M. J., Benecha, H. K., ... and A. H. Hurlbert. 2007. Species abundance distributions: moving beyond single prediction theories to integration within an ecological framework. Ecology letters, 10: 995-1015.

11. Preston, F. W. 1962. The canonical distribution of commonness and rarity: Part I. Ecology, 43:185-215.

12. Putnam, R. Community Ecology. 1993. Chapman & Hall, London, United Kingdom.

13. Shoemaker, W. R., Locey, K. J., & Lennon, J. T. 2017. A macroecological theory of 	microbial biodiversity. Nature Ecology & Evolution, 1:0107.

14. Taylor, L. R. 1961. Aggregation, variance and the mean. Nature. 189:4766 732–735.
