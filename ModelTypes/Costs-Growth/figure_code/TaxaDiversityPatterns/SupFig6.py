...
def figplot(x, y, xlab, ylab, fig, n):
    ...
    plt.scatter(x, y, color='0.3', s=sz, linewidths=0.1, edgecolor='k', alpha=0.5)
    plt.tick_params(axis='both', labelsize=4)
    plt.xlabel(xlab, fontsize=10)
    plt.ylabel(ylab, fontsize=8)

    plt.plot([-5, 1], [-5, 1], lw=0.5, color='k')
    plt.xlim(-5, 0)
    plt.ylim(-2.6, 0.3)
    return fig
...
plt.savefig(mydir + '/ModelTypes/Costs-Growth/results/figures/Supplement/SupFig6.png', dpi=200, bbox_inches = "tight")
plt.close()
