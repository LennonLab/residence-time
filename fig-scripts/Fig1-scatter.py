...
print dat.shape
print len(dat['individual.tau']),'rows'
print len(dat.columns),'columns'

dat['Itau'] = dat['individual.tau']
dat['Ptau'] = dat['particle.tau']
dat['Etau'] = dat['width']/dat['flow.rate']
...
plt.ylabel(r"$log_{10}$"+'(' + r"$\tau_{P}$" +')', fontsize=fs+3)
#plt.ylim(0, 3)
plt.xlabel(xlab, fontsize=fs+3)
plt.tick_params(axis='both', which='major', labelsize=fs)

plt.text(0, 1100,  r'$r^2$' + '=' +str(r2)+', '+r'$p$' + '=' +str(round(p2, 3)), fontsize=fs+1, color='k')

#### Itau vs. Tau #################################################################
fig.add_subplot(3, 3, 2)

dat['Itau'] = np.log10(dat['individual.tau'])
...
plt.text(1, 3.2,  r'$r^2$' + '=' +str(r2)+', '+r'$p$' + '=' +str(round(p3, 3)), fontsize=fs+1, color='k')
...
plt.xlabel(xlab, fontsize=fs+3)
plt.tick_params(axis='both', which='major', labelsize=fs)
plt.text(1, 3.2,  r'$r^2$' + '=' +str(r2)+', '+r'$p$' + '=' +str(round(p2,3)), fontsize=fs+1, color='k')
plt.ylim(0.5, 4)

#### Final Format and Save #####################################################
plt.subplots_adjust(wspace=0.4, hspace=0.4)
plt.savefig(mydir + '/results/figures/Fig1.png', dpi=600, bbox_inches = "tight")
#plt.show()
