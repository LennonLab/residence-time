...
def figplot(clrs, x, y, xlab, ylab, fig, n):
    ...
    plt.scatter(x2, y2, color = clrs, alpha= 1 , s = 15, linewidths=0.0, edgecolor='w')
...
df2['R'] = np.round(np.array(df['res.inflow'].groupby(df['sim']).mean()))
...
df2['Rp'] = np.log10(df['resource.particles'].groupby(df['sim']).mean())
...
#df2['B'] = df2['size'] * df2['G'] * df2['D'] * df2['E'] * df2['RPF'] * df2['MF'] * (1/df2['M'])

df2['B'] =  df2['size'] * df2['M']
