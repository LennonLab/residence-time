...
def ResIn(motion, Type, Vals, Xs, Ys, ID, IDs, t_In, numr, rmax, nN, nP, nC, w, h, u0):
    ...
    for r in range(numr):
        ...
        if x == 1:
            ...
            Xs.append(float(np.random.uniform(0.1*w, 0.25*w)))
    ...
    for m in range(seed):
        ...
        if x == 1:
            ...
            if prop not in colorD:
                ...
                P_RD[prop] = list(decomposition(1, nP))
                C_RD[prop] = list(decomposition(1, nC))
    ...
    for j in range(len(IDs)):
        ...
        if state == 'd':
            ...
            if x == 1:
                ...
                QN -= rpf*0.002 # becoming active costs energy
                QP -= rpf*0.002
                QC -= rpf*0.002
                Qs[i] = [QN, QP, QC]
...
def consume(RPFDict, R_Types, R_Vals, R_IDs, R_ID, R_Xs, R_Ys, R_t_In, R_xAge, Sp_IDs,
    ...
    for i, box in enumerate(I_Boxes):
        ...
        for ind in box: # The individuals
            ...
            QN -= mu1*0.007 # the faster you grow, the more energy it takes, and hence the more efficient you need to be
            QP -= mu1*0.007
            QC -= mu1*0.007
            ...
            if R_val > muQ: # Increase cell quota
                R_val = R_val - muQ
                Q += muQ
                ...
                if p == 1: # the environment is suitable for reproduction
                    ...
                    if p == 1:
                        ...
                        if p == 1:
                            ...
                            # A set of specific growth rates for three major types of resources
                            P_RD[spID_new] = list(decomposition(1, nP))
                            C_RD[spID_new] = list(decomposition(1, nC))
                            N_RD[spID_new] = list(decomposition(1, nN))
